...
for qu in idis:
    ...
    #             parent_visited = 1
    #             tmp_ques = []
    #             kids = children(crnt_tgs)
    #             [tree_tags.append(copy.deepcopy([qu, x, 1, 2])) for x in kids]
    ...
    #                     tree_tags = remove_duplicates(tree_tags)
    #                     break
    #                 sim_parent = parent(crnt_tgs, len(crnt_tgs) - h_parent, crnt_ans)
    #                 try:
    #                     [tree_tags.append(copy.deepcopy([qu, x, (1-(1/(h - h_parent))), 4])) for x in sim_parent]
    #                 except:
    #                     [tree_tags.append(copy.deepcopy([qu, x, 0, 4])) for x in sim_parent]
    ...
    #         if len(bag) > 1:
    #             rmed = bag[len(bag)-(i+1)]
    #             if rmed == '':
    #                 continue
    #             col = seman_tags[rmed].values.tolist()
    ...
    #             new_bg = []
    #             new_bg.extend(bag)
    #             new_bg.append(tg_set)
    #             if size >= 1:
    #                 tmp_bag = tag_obs_len1.loc[tag_obs_len1['tag1'].isin(bag)].values.tolist()
    #                 [qu_bag.append(copy.deepcopy(x)) for x in tmp_bag]
    #             if size >= 2:
    #                 tmp_bag = tag_obs_len2.loc[(tag_obs_len2['tag1'].isin(new_bg)) & (tag_obs_len2['tag2'].isin(new_bg))].values.tolist()
    #                 [qu_bag.append(copy.deepcopy(x)) for x in tmp_bag]
    #             if size >= 3:
    #                 tmp_bag = tag_obs_len3.loc[(tag_obs_len3['tag1'].isin(new_bg)) & (tag_obs_len3['tag2'].isin(new_bg)) & (tag_obs_len3['tag3'].isin(new_bg))].values.tolist()
    #                 [qu_bag.append(copy.deepcopy(x)) for x in tmp_bag]
    #             if size >= 4:
    #                 tmp_bag = tag_obs_len4.loc[(tag_obs_len4['tag1'].isin(new_bg)) & (tag_obs_len4['tag2'].isin(new_bg)) & (tag_obs_len4['tag3'].isin(new_bg)) & (tag_obs_len4['tag4'].isin(new_bg))].values.tolist()
    #                 [qu_bag.append(copy.deepcopy(x)) for x in tmp_bag]
    #             if size == 5:
    #                 tmp_bag = tag_obs_len5.loc[(tag_obs_len5['tag1'].isin(new_bg)) & (tag_obs_len5['tag2'].isin(new_bg)) & (tag_obs_len5['tag3'].isin(new_bg)) & (tag_obs_len5['tag4'].isin(new_bg)) & (tag_obs_len5['tag5'].isin(new_bg))].values.tolist()
    #                 [qu_bag.append(copy.deepcopy(x)) for x in tmp_bag]

    #             qu_bag = [x[1:2] for x in qu_bag] 
    ...
    #         bag.append(copy.deepcopy(crnt_tg))
