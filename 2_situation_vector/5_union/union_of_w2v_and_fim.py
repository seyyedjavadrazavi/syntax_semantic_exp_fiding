...
#     print('The id isssssssssssssssss: ',qu_id)
#     cnd = []
#     sim_itms = union.loc[union['qu_id'] == qu_id]
    
#     freq_itms = sim_itms.loc[sim_itms['origin'] == 'fim']
#     freq_itms =freq_itms.drop_duplicates(subset=['qu_id', 'sim_ques_id'])

#     w2v_itms = sim_itms.loc[sim_itms['origin'] == 'w2v']
#     w2v_itms = w2v_itms.drop_duplicates(subset=['qu_id', 'sim_ques_id'])
    
#     b_s_itms = b_s.loc[b_s['qu_id'] == qu_id]
#     b_s_itms = b_s_itms.drop_duplicates(subset=['qu_id', 'sim_ques_id'])

#     commn_itms = list(set(freq_itms.sim_ques_id) & set(w2v_itms.sim_ques_id) & set(b_s_itms.sim_ques_id))

#     if len(commn_itms) != 0:
    
#         cmmn_freq_itms = freq_itms.loc[freq_itms['sim_ques_id'].isin(commn_itms)] 
#         cmmn_w2v_itms = w2v_itms.loc[w2v_itms['sim_ques_id'].isin(commn_itms)] 
#         cmmn_b_s_itms = b_s_itms.loc[b_s_itms['sim_ques_id'].isin(commn_itms)] 
        
#         sum_cmmn_itms = reduce(lambda a, b: a.add(b, fill_value=0), [cmmn_freq_itms.similarity ,cmmn_w2v_itms.similarity, cmmn_b_s_itms.similarity])
#         sum_cmmn_itms = sum_cmmn_itms / 3
#         cmn_df = cmmn_w2v_itms[['qu_id', 'sim_ques_id']]
#         cmn_df['similarity'] = sum_cmmn_itms

#         nt_cmmn_freq_itms = freq_itms.loc[~freq_itms['sim_ques_id'].isin(commn_itms)] 
#         nt_cmmn_w2v_itms = w2v_itms.loc[~w2v_itms['sim_ques_id'].isin(commn_itms)] 
#         nt_cmmn_b_s_itms = b_s_itms.loc[~b_s_itms['sim_ques_id'].isin(commn_itms)]
...
#     else:
#         itms = pd.concat([freq_itms, w2v_itms, b_s_itms], ignore_index=True)
#         itms['similarity'] = itms['similarity'] / 3
#         itms.sort_values(by=['similarity'])
#         cnd = itms.iloc[:20]
#         cnd = cnd[['qu_id', 'sim_ques_id', 'similarity']]
...
#     cnd = cnd.values.tolist()
#     with open('../../data/bioinformatics/bioinformatics_avg_scores.csv', 'a', newline='') as f_object:              
#         writer_object = writer(f_object)
